/*networks.c
		...
		Contains functions to support the setup, sending and receiving on UDP
		Sockets... also contains the select_call() function */
...
int32_t udp_client_setup(char *hostname, uint16_t port_num, Connection *connection) {
	/* returns pointer to a sockadr_in that it created or NULL if host not found */
